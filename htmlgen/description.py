...
class _Text(_Node):
    """Represent a text node in a documentation."""
...
class _List(_Node):
    """Represent a sequence of documentation nodes."""
...
class _ElementRenderer(intermediate_rendering.DocutilsElementTransformer[_NodeUnion]):
    ...
    def __init__(
        ...
        """Initialize with the given values."""
        self.constraint_href_map = constraint_href_map
    ...
    def transform_reference_to_attribute_in_doc(
        ...
        return (
            _Element(
                name="a",
                attrs=collections.OrderedDict([("href", href)]),
                children=_List(items=[_Text(text)]),
            ),
            None,
        )
    ...
    def transform_reference_to_constraint_in_doc(
        ...
        constraint = element.reference
        href = self.constraint_href_map.get(constraint, None)
        assert href is not None, f"No href found for constraint: {constraint}"

        return (
            _Element(
                name="a",
                attrs=collections.OrderedDict([("href", href)]),
                children=_List(items=[_Text(constraint)]),
            ),
            None,
        )
    ...
    def transform_literal(
        ...
        return (
            _Element(
                name="code", children=_List(items=[_Text(content=element.astext())])
            ),
            None,
        )
    ...
    def transform_paragraph(
        ...
        return _Element(name="p", children=children), None
    ...
    def transform_note(
        ...
        return (
            _Element(
                name="div",
                attrs=collections.OrderedDict([("class", "alert alert-info")]),
                children=children,
            ),
            None,
        )
    ...
    def transform_reference(
        ...
        assert len(element.children) == 1 and isinstance(
            element.children[0], docutils.nodes.Text
        ), (
            f"Expected the children of a reference to be a single text node, "
            f"but got an element: {element}"
        )

        return (
            _Element(
                name="a",
                attrs=collections.OrderedDict([("href", element.children[0].astext())]),
                children=_List([_Text(element.children[0].astext())]),
            ),
            None,
        )
...
class _RemoveRedundantParagraphVisitor(_NodeVisitor):
    """Remove the redundant ``<p>`` elements in-place."""
...
def _compress_node_in_place(node: _NodeUnion) -> None:
    ...
    remove_redundant_paragraph_visitor = _RemoveRedundantParagraphVisitor()
    remove_redundant_paragraph_visitor.visit(node)
...
def _render_summary_remarks(
    ...
    element_renderer = _ElementRenderer(constraint_href_map=constraint_href_map)
    ...
    if summary_errors is not None:
        ...
        result_items.append(
            _Element(
                name="div",
                attrs=collections.OrderedDict([("class", "summary")]),
                children=_List(items=[summary_node]),
            )
        )
    ...
    if len(remark_nodes) > 0:
        result_items.append(
            _Element(
                name="div",
                attrs=collections.OrderedDict([("class", "remarks")]),
                children=_List(items=remark_nodes),
            )
        )
...
def _render_summary_remarks_constraints(
    ...
    for identifier, docutils_element in description.constraints_by_identifier.items():
        ...
        if body_errors is not None:
                ...
                _Element(
                    ...
                    children=_List(
                        items=[
                            ...
                            _Text(f"Constraint {identifier}"),
                            _Element(
                                name="a",
                                attrs=collections.OrderedDict(
                                    [
                                        ("class", "aas-anchor-link"),
                                        ("href", f"#constraint-{identifier}"),
                                    ]
                                ),
            ...
            constraint_nodes.append(_Element(name="dd", children=_List(items=[body])))
    ...
    if len(constraint_nodes) > 0:
        result_items.append(
            _Element(
                name="dl",
                attrs=collections.OrderedDict([("class", "constraints")]),
                children=_List(items=constraint_nodes),
            )
        )
...
class _ToTextDirectivesVisitor(_NodeVisitor):
    ...
    The text is expected to be valid HTML and properly escaped.
    """
    ...
    def visit_text(self, node: _Text) -> None:
        self._last_or_new_block().parts.append(html.escape(node.content))
    ...
    def visit_element(self, node: _Element) -> None:
        ...
        if node.name in ("pre", "code"):
            assert (
                node.children is None or len(node.children.items) <= 1
            ), "Expected at most 1 child in a <pre> or <code>"

            assert isinstance(
                node.children.items[0], _Text
            ), "Expected the content of the <pre> or <code> to be a text"

            self.directives.append(
                _TextBlock(
                    parts=[
                        start_element_writer.getvalue(),
                        node.children.items[0].content,
                        f"</{node.name}>",
                    ]
                )
            )
            ...
            # Inline <a> to avoid formatting errors where whitespace is wrongly
            # formatted as part of the link.
            ...
            self.directives.append(_RelativeIndention(1))
...
def _generate_summary_remarks(
    ...
    """Generate the documentation for a summary-remarks description."""
    node, errors = _render_summary_remarks(
        description=description, constraint_href_map=constraint_href_map
    )
    if errors is not None:
        return None, errors
...
def _generate_summary_remarks_constraints(
    ...
    """Generate the documentation for a summary-remarks-constraints."""
    node, errors = _render_summary_remarks_constraints(
        description=description, constraint_href_map=constraint_href_map
    )
    if errors is not None:
        return None, errors
...
def generate_for_meta_model(
    description: intermediate.DescriptionOfMetaModel,
    constraint_href_map: Mapping[str, str],
) -> Tuple[Optional[Stripped], Optional[List[Error]]]:
    """Generate the documentation for the given meta-model."""
    return _generate_summary_remarks_constraints(
        description=description, constraint_href_map=constraint_href_map
    )


def generate_for_our_type(
    description: intermediate.DescriptionOfOurType,
    constraint_href_map: Mapping[str, str],
) -> Tuple[Optional[Stripped], Optional[List[Error]]]:
    """Generate the documentation for our type."""
    return _generate_summary_remarks_constraints(
        description=description, constraint_href_map=constraint_href_map
    )
...
def generate_for_property(
    ...
    """Generate the documentation for the given property."""
    return _generate_summary_remarks_constraints(
        description=description, constraint_href_map=constraint_href_map
    )


def generate_for_enumeration_literal(
    description: intermediate.DescriptionOfEnumerationLiteral,
    constraint_href_map: Mapping[str, str],
) -> Tuple[Optional[Stripped], Optional[List[Error]]]:
    """Generate the documentation for the given enumeration literal."""
    return _generate_summary_remarks(
        description=description, constraint_href_map=constraint_href_map
    )
...
def generate_for_constant(
    ...
    """Generate the documentation for the given constant."""
    return _generate_summary_remarks(
        description=description, constraint_href_map=constraint_href_map
    )
...
def _render_description_of_signature(
    ...
    """Render a description for a signature."""
    result_items = []  # type: List[_NodeUnion]
    errors = []  # type: List[Error]

    renderer = _ElementRenderer(constraint_href_map=constraint_href_map)
    ...
    for name, docutils_element in description.arguments_by_name.items():
        arg_body, body_errors = renderer.transform(docutils_element)
        if body_errors is not None:
            errors.extend(
                Error(description.parsed.node, message) for message in body_errors
            )
            ...
            args_and_return_nodes.append(
                _Element(name="dd", children=_List(items=[arg_body]))
            )
    ...
    if description.returns is not None:
        ...
        if returns_errors is not None:
            ...
            args_and_return_nodes.append(
                _Element(name="dd", children=_List(items=[returns]))
            )
    ...
    if len(args_and_return_nodes) > 0:
        result_items.append(
            _Element(
                name="dl",
                attrs=collections.OrderedDict([("class", "args-and-return")]),
                children=_List(items=args_and_return_nodes),
            )
        )
...
def generate_for_signature(
    signature_name: Identifier,
    description: intermediate.DescriptionOfSignature,
    constraint_href_map: Mapping[str, str],
) -> Tuple[Optional[Stripped], Optional[List[Error]]]:
    """
    Generate the documentation for the given signature.
